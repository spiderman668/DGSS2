...
if __name__ == '__main__':
    ...
    #writer = SummaryWriter('runs/loewe/without_cp/transductive')
    modeling = CBA
    ...
    comb_path = './data/loewe/transductive/loewe.feather'
    cell_path = os.path.join('./data', 'cell_lines.feather')
    with open('./data/loewe/transductive/loewe.json', 'r') as file:
        data_idx = json.load(file)

    print(modeling.__name__)
    print('use:', device)
    dataset = DrugCombDataset(comb_path, cell_path)
    ...
    for fold in range(4, 5):
        ...
        selection = data_idx[f'fold_{fold}']
        train_idx = np.array(selection['train'])
        test_idx = np.array(selection['test'])
        # 五折交叉
        train_subset = Subset(dataset, train_idx)
        test_subset = Subset(dataset, test_idx)
        loader_train = DataLoader(train_subset, batch_size=TRAIN_BATCH_SIZE, shuffle=True,pin_memory=True, num_workers=10,
                                  collate_fn=collate)
        loader_test = DataLoader(test_subset, batch_size=TEST_BATCH_SIZE, shuffle=False,pin_memory=True, num_workers=10,
                                 collate_fn=collate)
        loss_fn = nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=LR)

        file_result = './result2/loewe/transductive/fold' + str(fold) + '.csv'
        file_result2 = './result2/loewe/transductive/fold' + str(fold) + '_all' + '.csv'
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=30, gamma=0.1)
        # model_path = f'./model/global_attn/transductive/fold{fold}_best_model.pth'
